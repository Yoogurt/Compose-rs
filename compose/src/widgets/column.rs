...
pub trait ColumnScope {
    ...
    fn align(&self, alignment_horizontal: AlignmentHorizontal) -> Modifier;
}
...
fn horizontal_align_modifier(cross_axis_alignment: CrossAxisAlignment) -> Modifier {
    Modifier::ModifierNodeElement {
        create: (move || {
            HorizontalAlignModifier::new() as Rc<RefCell<dyn ModifierNode>>
        }).wrap_with_box(),
        update: (move |mut modifier_node: RefMut<dyn ModifierNode>| {
            // if let Some(row_column_parent_data) = modifier_node.as_any_mut().downcast_mut::<RowColumnParentData>() {
            //     row_column_parent_data.weight = weight;
...
fn row_column_measure_policy(
    ...
    cross_axis_aligment: CrossAxisAlignment,
