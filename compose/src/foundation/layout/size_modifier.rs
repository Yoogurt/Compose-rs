...
fn size_measure_policy<T>(min_width: T,
    ...
    Box::new(move |measure_scope: &mut dyn MeasureScope, measurable: &mut dyn Measurable, _constraint: &Constraints| -> MeasureResult {
...
impl LayoutModifierNode for SizeNode {
    fn measure(&mut self, measure_scope: &mut dyn MeasureScope, measurable: &mut dyn Measurable, constraints: &Constraints) -> MeasureResult {
        ...
        let wrapped_constraints = if self.enforce_incoming {
            ...
            let resolved_min_width = if self.min_width != Dp::UNSPECIFIC {
                ...
                constraints.min_width.coerce_at_most(target_constraints.max_width)
            };

            let resolved_max_width = if self.max_width != Dp::UNSPECIFIC {
                target_constraints.max_width
            } else {
                constraints.max_width.coerce_at_least(target_constraints.min_width)
            };

            let resolved_min_height = if self.min_height != Dp::UNSPECIFIC {
                target_constraints.min_height
            } else {
                constraints.min_height.coerce_at_most(target_constraints.max_height)
            };

            let resolved_max_height = if self.max_height != Dp::UNSPECIFIC {
                target_constraints.max_height
            } else {
                constraints.max_height.coerce_at_least(target_constraints.min_height)
            };

            ((resolved_min_width..=resolved_max_width), (resolved_min_height..=resolved_max_height)).into()
        };
...
fn size_element<T>(min_width_raw: T,
                   ...
                   min_height_raw: T,
                   max_height_raw: T,
                   enforce_incoming: bool) -> Modifier where T: Into<Dp> + Copy + 'static {
...
impl SizeModifier for Modifier {
    fn width(self, width: Dp) -> Modifier {
        self.then(size_element(width, width, Dp::UNSPECIFIC, Dp::UNSPECIFIC, false))
    }
}
